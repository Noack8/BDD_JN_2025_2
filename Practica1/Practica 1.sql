...
select MUNICIPIO_RES, YEAR(FECHA_INGRESO) as anio_Ingreso, count(*) as total_Recuperados --Sacamo los datos para que se vea shido
...
group by MUNICIPIO_RES, YEAR(FECHA_INGRESO)	--Agrupamos para poder valorar lo resultados
having  count(*) = ( --Autoreunion
	select max(total_Recuperados) --Escogemos los valores maximos contados por año
	from(
		select MUNICIPIO_RES, YEAR(FECHA_INGRESO) as anio_Ingreso, count(*) as total_Recuperados --Resultados de interes
		...
		group by MUNICIPIO_RES, YEAR(FECHA_INGRESO)	--Con ello podemos juntar los valores a tomar en cuenta
	)as T1
